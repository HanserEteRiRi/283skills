...
- 质量自检（Phase 4用）：`python3 [skill目录]/scripts/quality_check.py <SKILL.md路径>`
  - 自动检查8项通过标准：心智模型数量、局限性、表达DNA、知识边界、剧情记忆、诚实边界、内在张力、一手来源占比
  - 输出逐项PASS/FAIL和总结
...
人物Skill，尤其是虚构角色Skill，必须额外提炼“角色知道什么”和“角色不该知道什么”。这是防止模型用通用知识库替角色装懂的关键。

必须产出 `references/knowledge/knowledge-boundary.md`，记录：

- 角色年龄/学历/职业身份/所属组织。
- 角色可以直接回答的范围：自身经历、亲密关系、公开剧情、本地语料中出现的事件、符合年龄和学历的日常常识。
- 角色必须承认不知道的范围：医疗、法律、投资、编程、专业学术、现代新闻、政治、角色未接触的行业知识、其他角色未公开的内心、未记录剧情。
- 角色拒答时的口吻：用角色语言说“不懂/不知道/没学过/不记得”，不要跳出角色。
- 剧情记忆索引：关键剧情节点、时间线、人物关系和本地语料报告入口。
...
python scripts/build_character_knowledge_pack.py \
  --skill-dir <skill目录> \
  --profile-config <角色profile.json> \
  --boundary-config <知识边界config.json>
```
...
| 知识边界与角色记忆 | Phase 2.7 结果 + `references/knowledge/knowledge-boundary.md` |
...
| 知识边界 | 明确角色按年龄/学历/经历知道什么，不知道时会拒答 | 角色用通用模型知识硬答 |
| 剧情记忆 | 关键剧情、关系和来源有可读入口 | 只会模仿口吻，不记得剧情发展 |
