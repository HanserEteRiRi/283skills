...
def check_knowledge_boundary(content: str) -> tuple[bool, str]:
    """检查角色知识边界：避免角色用模型通用知识越界回答。"""
    has_section = bool(re.search(r'知识边界|知识范围|Scope Gate|范围门禁|知识门禁', content, re.IGNORECASE))
    boundary_markers = len(
        re.findall(
            r'年龄|学历|年级|经历|自身相关|超出范围|不知道|不懂|不会|不能装作|不硬答|拒答|退出角色',
            content,
        )
    )
    passed = has_section and boundary_markers >= 4
    return passed, (
        f"知识边界标记: {boundary_markers}项 {'✅' if passed else '❌ (需明确角色知道/不知道什么)'}"
    )


def check_character_memory(content: str) -> tuple[bool, str]:
    """检查剧情/人物记忆索引：角色应能定位自身剧情，而不是只模仿口吻。"""
    has_memory = bool(re.search(r'剧情记忆|角色记忆|人物时间线|character memory|plot memory', content, re.IGNORECASE))
    memory_refs = len(
        re.findall(
            r'references/(?:knowledge|research|local-dialogue-analysis)|source-index|dialogue-report|剧情|时间线',
            content,
            re.IGNORECASE,
        )
    )
    passed = has_memory and memory_refs >= 3
    return passed, f"剧情记忆索引: {memory_refs}处 {'✅' if passed else '❌ (需记录剧情来源和读取入口)'}"
...
def main():
    ...
    checks = [
        ...
        ("知识边界", check_knowledge_boundary),
        ("剧情记忆", check_character_memory),
